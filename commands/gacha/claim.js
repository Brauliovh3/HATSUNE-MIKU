  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (chat.adminonly || !chat.gacha) {
        return m.reply(`🎤 ${global.miku || 'HATSUNE MIKU'} Los comandos de *Gacha* están desactivados en este grupo.\n\nUn *administrador* puede activarlos con:\n» *${usedPrefix}gacha on*`)
      }
      ...
      if (me.lastClaim && now < me.lastClaim) {
        ...
        return m.reply(`💙 ${global.miku || 'HATSUNE MIKU'} Debes esperar *${timeText.trim()}* para usar *${command}* de nuevo.`)
      }
      const quotedId = m.quoted?.id
      if (!quotedId || !chat.rolls[quotedId]) {
        return m.reply(`🎤 ${global.miku || 'HATSUNE MIKU'} Debes citar un personaje válido para reclamar.`)
      }
      const rollData = chat.rolls[quotedId]
      const id = rollData.id
      const structure = await loadCharacters()
      const sourceData = getCharacterById(id, structure)
      if (!sourceData) return m.reply(`🎤 ${global.miku || 'HATSUNE MIKU'} Personaje no encontrado en characters.json`)
      ...
      if (record.reservedBy && record.reservedBy !== m.sender && now < record.reservedUntil) {
        ...
        return m.reply(`💙 ${global.miku || 'HATSUNE MIKU'} Este personaje está protegido por *${reserverName}* durante *${remaining}s.*`)
      }
      if (record.expiresAt && now > record.expiresAt && !record.user && !(record.reservedBy && now < record.reservedUntil)) {
        ...
        return m.reply(`🎤 ${global.miku || 'HATSUNE MIKU'} El personaje ha expirado » ${expiredTime}s.`)
      }
      if (record.user) {
        const ownerName = global.db.data.users[record.user]?.name || `@${record.user.split('@')[0]}`
        return m.reply(`💙 ${global.miku || 'HATSUNE MIKU'} El personaje *${record.name}* ya ha sido reclamado por *${ownerName}*`)
      }
      ...
      await client.sendMessage(m.chat, { text: `🎤 ${global.miku || 'HATSUNE MIKU'} ${finalMessage} (${duration}s)` }, { quoted: m })
