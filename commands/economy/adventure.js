export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (chat.adminonly || !chat.economy) return client.reply(m.chat, `💙 ${global.miku || 'HATSUNE MIKU'} Los comandos de *Economía* están desactivados en este grupo.\n\nUn *administrador* puede activarlos con el comando:\n» *${usedPrefix}economy on*`, m)    
    user.lastadventure ||= 0
    if (user.coins == null) user.coins = 0
    if (user.health == null) user.health = 100
    if (user.health < 5) return m.reply(`🌱 ${global.miku || 'HATSUNE MIKU'} No tienes suficiente salud para volver a *aventurarte*.\n> Usa *"${usedPrefix}heal"* para curarte.`)
    const remainingTime = user.lastadventure - Date.now()
    if (remainingTime > 0) {
      return client.reply(m.chat, `💙 ${global.miku || 'HATSUNE MIKU'} Debes esperar *${msToTime(remainingTime)}* antes de volver a aventurarte.`, m)
    }
