...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (!who2) return m.reply(`💙 ${global.miku || 'HATSUNE MIKU'} Debes mencionar a quien quieras transferir *${monedas}*.\n> Ejemplo » *${usedPrefix + command} 25000 @mencion*`)
    const who = await resolveLidToRealJid(who2, client, m.chat)
    const senderData = chatData.users[m.sender]
    const targetData = chatData.users[who]
    if (!targetData) return m.reply(`🌱 ${global.miku || 'HATSUNE MIKU'} El usuario mencionado no está registrado en El bot.`)
    ...
    if (!cantidadInput || isNaN(cantidad) || cantidad <= 0) {
      return m.reply(`💙 ${global.miku || 'HATSUNE MIKU'} Ingresa una cantidad válida de *${monedas}* para transferir.`)
    }
    if (typeof senderData.bank !== 'number') senderData.bank = 0
    if (senderData.bank < cantidad) {
      return m.reply(`🌱 ${global.miku || 'HATSUNE MIKU'} No tienes suficientes *${monedas}* en el banco para transferir.\n> Tu saldo actual: *¥${senderData.bank.toLocaleString()} ${monedas}*`)
    }
    ...
    await client.sendMessage(chatId, { text: `💙 ${global.miku || 'HATSUNE MIKU'} Transferiste *¥${cantidad.toLocaleString()} ${monedas}* a *${name}*\n> Ahora tienes *¥${senderData.bank.toLocaleString()} ${monedas}* en tu banco.`, mentions: [who] }, { quoted: m })
  }
}