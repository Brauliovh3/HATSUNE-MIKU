...
export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (!target) return m.reply(`🌱 ${global.miku || 'HATSUNE MIKU'} El usuario no se encuentra en la base de Datos.`)
    ...
    target.health = 100
    const info = who ? `💙 ${global.miku || 'HATSUNE MIKU'} Has curado a *${db.users[who]?.name || who.split('@')[0]}* hasta el máximo nivel de salud.` : `🌱 ${global.miku || 'HATSUNE MIKU'} Te has curado hasta el máximo nivel de salud.`
    m.reply(info)
  },
}